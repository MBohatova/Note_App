...
/* 
react/vue - декларативний підхід (концентрується на результаті)
js - імперативинй (покроково описуємо що ми хочемо отримати)
*/

let createNoteFormContainer = document.getElementById('form_container')
...
function generateNote() {
  ...
  notesList.forEach((note) => {
    ...
    notesWrapper.insertAdjacentHTML('afterbegin',
      `<div id="${note.id}" class="main__note ${colorTag}">
        <div class="main__headlineAndButtonWrapper">
          ...
          <button class="main__deleteButton" id="${note.date}_${note.title}"></button>
        </div>
    ...
    let deleteCurrentNoteBtn = document.getElementById(`${note.date}_${note.title}`);
    deleteCurrentNoteBtn.onclick = (event) => {
...
const deleteNoteFromLocalStorage = (noteId) => {
  ...
  if (notesFromStorage) {
    ...
    let newNotesArr = storageArr.filter(note => note.id !== noteId);
    localStorage.notesArr = JSON.stringify(newNotesArr);
  }
}
...
function deleteNote(event, noteId) {
  ...
  function onDeleteButtonHandler() {
    let notesFromStorage = localStorage.getItem('notesArr');
    let storageArr = JSON.parse(notesFromStorage);
    let noteI = storageArr.findIndex(storedNote => storedNote.id === parseInt(noteElement.id));//noteId
